#!/usr/bin/env python3
# ═══════════════════════════════════════════════════════════════════════════
# validate_palettes — validates this palette set without external dependencies.
#
#   - Every <slug>.json matches the contract (name/slug/base16 + hex).
#   - index.json lists exactly the existing slugs (the desktop panel uses
#     index.json as its card model).
#
...
PALETTES = pathlib.Path(__file__).resolve().parent.parent
...
def check_index(slugs: set) -> None:
    ...
    for slug in sorted(slugs - listed):
        errors.append("%s.json is not in index.json" % slug)
